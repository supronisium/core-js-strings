...
function sumOfCodes(str) {
  const inputString = str == null ? '' : str;
  let sum = 0;
...
function countVowels(str) {
  const matchesSymbols = str.match(/[aeiouyAEIOUY]/g);
  return matchesSymbols ? matchesSymbols.length : 0;
}
/**
...
function isPalindrome(str) {
  const palidromeString = str.replace(/[^a-z0-9]/gi, '').toLowerCase();
  return palidromeString === palidromeString.split('').reverse().join('');
}
/**
...
function reverseWords(str) {
  ...
  const reversedWords = aWords.map((word) => word.split('').reverse().join(''));
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (char === char.toUpperCase()) {
      Invert += char.toLowerCase();
    } else {
      Invert += char.toUpperCase();
    }
  }
  return Invert;
}
...
function encodeToRot13(str) {
  const letterLow = 'abcdefghijklmnopqrstuvwxyz';
  const letterUp = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  return str
    .split('')
    .map((char) => {
      if (letterLow.includes(char)) {
        const index = (letterLow.indexOf(char) + 13) % 26;
        return letterLow[index];
      }
      if (letterUp.includes(char)) {
        const index = (letterUp.indexOf(char) + 13) % 26;
        return letterUp[index];
      }
      return char;
    })
    .join('');
